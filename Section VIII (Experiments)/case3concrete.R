...
library(neuralnet)
library(readxl)
DataSet<-read_excel('/home/n10141065/mae/10leave/nucase3/Concrete_Data.xls')
...
for (cv_nu_Value in c(1:10)/10){ 
  ...
  for (cv_C_value in 10^c(-3:3) ){
    
    for (cv_std_Value in 10^c(-2:2)){
      
      
  
EpsSVR<-svm(x_train,y_train,type='nu-regression',kernel="radial",scale=FALSE, nu=cv_nu_Value, 
            cost=cv_C_value, gamma=1/(2*cv_std_Value^2))
